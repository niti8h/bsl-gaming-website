...
<h2 style="color: var(--royal-gold); font-size: 24px; font-weight: 600; margin-bottom: 20px;">Edit Testimonial</h2>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="content" id="content" rows="5" required
                style="width: 100%; padding: 12px; background-color: var(--dark-bg); border: 1px solid var(--dark-border); color: #fff; border-radius: 4px; font-family: inherit; resize: vertical;">{{ old('content', $testimonial->content) }}</textarea>
            @error('content')
                <div style="color: #dc3545; margin-top: 5px; font-size: 14px;">{{ $message }}</div>
            @enderror
        </div>
